...
class flockRobot1(Entity):
    def __init__(self,id):
        ...
        # For communications
        self.commsRadius = 0.1       #width of detection radius
        self.numberOfNeighbours = 5     #number of neighbours to see in comms radius
        self.dataOfNeighbours = np.zeros([self.numberOfNeighbours,4]) 
        self.numberOfNeighbours_normal = 5 
        self.dataOfNeighbours_normal = np.zeros([self.numberOfNeighbours_normal,4])
        ...
        self.anglei = np.random.uniform(0,360)
        self.cosAngle = math.cos(math.radians(self.anglei))   #for x direction #for random tumble angle
        # double cosAngle   #for x direction
        self.sinAngle = math.sin(math.radians(self.anglei)) #for y direction
        self.previousAngle = self.anglei  #for forward motion
        ...
        self.shepherdXVelocity = 0.0
        self.shepherdYVelocity = 0.0

        self.last_vm = np.zeros([2,1])
        self.bac_state = 0  # 0 is the normal bac, 1 is the highest bac but no communication between them
    ...
    def shepherdForce(self):	
        ...
        # print(self.dataOfNeighbours)
        for x in range(5):
            if self.dataOfNeighbours_normal[x][1] != 0:
                counterX = counterX + 1
            if self.dataOfNeighbours_normal[x][2] != 0:
                counterY = counterY + 1 
        ...
        for x in range(5):
            averageX = self.dataOfNeighbours_normal[x][1] + averageX 
            averageY = self.dataOfNeighbours_normal[x][2] + averageY 
    ...
    def match_velocity(self, world):
        ...
        for id in range(len(world.agents)):
            if self.idNo !=  (world.robotInfo[id,0]):
                if (world.robotInfo[id,1] < (self.state.p_pos[0] + self.commsRadius)) and (world.robotInfo[id,1] > (self.state.p_pos[0] - self.commsRadius)) and (world.robotInfo[id,2] < (self.state.p_pos[1] + self.commsRadius)) and (world.robotInfo[id,2] > (self.state.p_pos[1] - self.commsRadius)) and self.bac_state == 1:
                     #store the x and y coordinates of robots within the comms radius
	...
	# normal agents will push highest agents to cover more area 
    def shepherding(self,world):
        #collect normal bac in the view
        neighbourData = np.zeros([self.numberOfNeighbours_normal, 4])
        counter = 0
		
        for id in range(len(world.agents)):
            if self.idNo !=  (world.robotInfo[id,0]):
                if (world.robotInfo[id,1] < (self.state.p_pos[0] + self.commsRadius)) and (world.robotInfo[id,1] > (self.state.p_pos[0] - self.commsRadius)) and (world.robotInfo[id,2] < (self.state.p_pos[1] + self.commsRadius)) and (world.robotInfo[id,2] > (self.state.p_pos[1] - self.commsRadius)) and world.agents[id].bac_state == 0:
                     #store the x and y coordinates of robots within the comms radius
                    neighbourData[counter,0] = world.robotInfo[id,0]
                    neighbourData[counter,1] = world.robotInfo[id,1]-self.state.p_pos[0]
                    neighbourData[counter,2] = world.robotInfo[id,2]-self.state.p_pos[1]
                    neighbourData[counter,3] = world.robotInfo[id,3]
                    ...
                    if counter >= self.numberOfNeighbours_normal:
                        counter = 0
                        break

        self.dataOfNeighbours_normal = neighbourData
        shepherd_force = np.zeros(2)
        shepherd_force = self.shepherdForce()

        self.shepherdXVelocity =  shepherd_force[0]#+ checkBounds[0] 	 # Combine the rules
        self.shepherdYVelocity =  shepherd_force[1] #+ checkBounds[1]

        if np.abs(self.shepherdXVelocity) > 0.01:
            self.shepherdXVelocity = (self.shepherdXVelocity/np.abs(self.shepherdXVelocity))*0.01

        if np.abs(self.shepherdYVelocity) > 0.01:
            self.shepherdYVelocity = (self.shepherdYVelocity/np.abs(self.shepherdYVelocity))*0.01
    ...
    def flockingController2(self,world):
        ...
        # self.myXVelocity = (0.8 * sepDist[0]  + 0.52 * attractDist[0] + 0.1*mVel[0]) #+ checkBounds[0] 	 # Combine the rules
        # self.myYVelocity = (0.8 * sepDist[1]  + 0.52 * attractDist[1] + 0.1*mVel[1]) #+ checkBounds[1]

        self.myXVelocity = (0.5 * sepDist[0]  + 0.52 * attractDist[0] + 0.1*mVel[0]) #+ checkBounds[0] 	 # Combine the rules
        self.myYVelocity = (0.5 * sepDist[1]  + 0.52 * attractDist[1] + 0.1*mVel[1]) #+ checkBounds[1]

        # print("sep",sepDist,"attact",attractDist,"mat", mVel)
        ...
        if np.abs(self.myYVelocity) > 0.01:
            self.myYVelocity = (self.myYVelocity/np.abs(self.myYVelocity))*0.01
        # print(self.myXVelocity,self.myYVelocity)
    ...
    def receiveNeighbourData(self,world):
        ...
        for id in range(len(world.agents)):
            if self.idNo !=  (world.robotInfo[id,0]):
                if (world.robotInfo[id,1] < (self.state.p_pos[0] + self.commsRadius)) and (world.robotInfo[id,1] > (self.state.p_pos[0] - self.commsRadius)) and (world.robotInfo[id,2] < (self.state.p_pos[1] + self.commsRadius)) and (world.robotInfo[id,2] > (self.state.p_pos[1] - self.commsRadius)) and world.agents[id].bac_state == 1:
                     #store the x and y coordinates of robots within the comms radius
                    neighbourData[counter,0] = world.robotInfo[id,0]
                    neighbourData[counter,1] = world.robotInfo[id,1]-self.state.p_pos[0]
                    neighbourData[counter,2] = world.robotInfo[id,2]-self.state.p_pos[1]
    ...
    def bacteriaController00(self):
		...
		#calculate dP/dt 
		#*****************Checking ABS and not ABS ********** #
        dP_Dt = (kd / ((kd + presentReading) * (kd + presentReading)) * (presentReading - self.previousReading)*5000) 	
       	# print("diff",(presentReading - self.previousReading))
        presentReadNStore = presentReading 
        ...
        self.wDP_Dt =  (1 / Tm) * self.wDP_Dt 
        # print("wawa", self.wDP_Dt)
		
        if self.myData[3] == 0:
            self.varyNTumbleLength = self.varyNTumbleLength + 0.2
            if self.varyNTumbleLength > 140:
                self.varyNTumbleLength = 140 
            stdTumbleLenght = 60	
        elif self.myData[3] > 0:
            self.varyNTumbleLength = 0 
            stdTumbleLenght = 60
		
        self.tumbleLenght = (int(self.varyNTumbleLength) + stdTumbleLenght) * np.exp(alpha * self.wDP_Dt)   #increase rate of random direction
        # print(math.exp(alpha * self.wDP_Dt))
        # if int(self.varyNTumbleLength) == 0:
        #     # print("wawa",self.tumbleLenght)

        stdVelocity = 30000000
        # vDP_Dt = (vKd / (presentReadNStore))  # * (previousReading - presentReading) 
        # print(vDP_Dt)
        self.bactVelocity = (stdVelocity)*0.01/(presentReadNStore*presentReadNStore)     #reduce velocity as your get closer to source and to stay in source
        # print(self.bactVelocity)		
        if self.bactVelocity > 0.01:
            self.bactVelocity = 0.01
...
class World(object):
    ...
    def check_state(self):
        for agent in self.agents:
            if agent.bac_state == 0:
                agent.color = np.array([2.5,0,0])
            else:
                agent.color = np.array([0,0,2.5])
    ...
    def check_in_peanut(self, pos):
        ...
        d1 = np.sqrt(((pos-pos1)**2).sum())
        d2 = np.sqrt(((pos-pos2)**2).sum())
    ...
    def swarm_trigger(self):
        ...
        # return dsum
        if self.p_shape == "circle_nonuni":
            cx = np.zeros(len(self.agents))
            cy = np.zeros(len(self.agents))
            check_inside = np.zeros(len(self.agents))

            for i in range(len(self.agents)):
                pos = self.agents[i].state.p_pos
                cx[i] = pos[0]
                cy[i] = pos[1]
                check_inside[i] = self.check_in(pos)
            ...
            high_state = np.zeros(len(self.agents))
            for i in range(len(self.agents)):
                high_state[i] = self.agents[i].bac_state

            if d < 0.003 and diff < 0.0001 and (np.sum(check_inside)==len(self.agents)) and (np.sum(high_state)==len(self.agents)):
                signal = 1
            else:
                signal = 0
        ...
        if self.p_shape == "peanut_uni":
            ...
            for i in range(len(self.agents)):
                pos = self.agents[i].state.p_pos
                check_inside[i],lcc,rcc = self.check_in_peanut(pos)
            ...
            cen_compare_l = np.array([-0.2,0]) 
            cen_compare_r = np.array([0.2,0])
            ...
            diffl = np.sqrt(((centroidl - self.old_centroid)**2).sum())
            diffr = np.sqrt(((centroidr - self.old_centroid1)**2).sum())

            diff = diffl+diffr
            d = dl+dr
            self.old_centroid = centroidl
            self.old_centroid1 = centroidr
            ...
            if d < 0.07 and diff < 0.0002 and (np.sum(check_inside)==len(self.agents)) and (np.sum(high_state)==len(self.agents)):
        ...
        if self.p_shape == "square_uni":
            ...
            if d < 0.003 and diff < 0.0001 and (np.sum(check_inside)==len(self.agents)):
                signal = 1
            else:
                signal = 0
